...
def my_lstm_layer(input_reps, lstm_dim, input_lengths=None, scope_name=None, reuse=False, is_training=True,
	...
	with tf.variable_scope(scope_name, reuse=reuse):
		...
		# 	lstm = tf.contrib.cudnn_rnn.CudnnLSTM(1, lstm_dim, direction="bidirectional", dropout=0.0)
		...
		# else:
			...
			context_lstm_cell_fw = tf.nn.rnn_cell.DropoutWrapper(context_lstm_cell_fw, output_keep_prob=dropout_rate)
			context_lstm_cell_bw = tf.nn.rnn_cell.DropoutWrapper(context_lstm_cell_bw, output_keep_prob=dropout_rate)
...
def dropout_layer(input_reps, dropout_rate, is_training=True):
	output_repr = tf.nn.dropout(input_reps, (dropout_rate)) # keep_prob
	return output_repr
