...
def task_specific_attention(inputs, output_size, input_mask,
    ...
    with tf.variable_scope(scope or 'attention') as scope:
        ...
        vector_attn_max = (tf.reduce_max(vector_attn, axis=1, keep_dims=True))
...
def cosine_constrastive(sentence_one, sentence_two, y_input):
    ...
    with tf.name_scope("l2_similarity"):
        ...
        y_true = tf.cast(tf.argmax(y_input, axis=1), tf.float32)
        contrastive_distance = y_true * (distance) + (1 - y_true) *(tf.maximum((0.05 - distance), 0))
...
def BiRNN(x, dropout, scope, sequence_length, hidden_units, is_train, rnn_cell="lstm_cell"):
    """use dynamic rnn without reshaping input
    ...
    n_layers=1
    # add new recurrent cell
        
    def create_bi_directional_encoder(rnn_cell):
        if rnn_cell == "lstm_cell":
            forward_cell = tf.contrib.rnn.LSTMCell(n_hidden, forget_bias=1.0, state_is_tuple=True, reuse=tf.get_variable_scope().reuse)
            backward_cell = tf.contrib.rnn.LSTMCell(n_hidden, forget_bias=1.0, state_is_tuple=True, reuse=tf.get_variable_scope().reuse)
                
        elif rnn_cell == "gru_cell":
            forward_cell = tf.contrib.rnn.GRUCell(n_hidden, reuse=tf.get_variable_scope().reuse)
            backward_cell = tf.contrib.rnn.GRUCell(n_hidden, reuse=tf.get_variable_scope().reuse)
                
        outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=forward_cell,
        ...
        forward_representation, backward_representation = outputs
        forward_representation = tf.nn.dropout(
                forward_representation,
                keep_prob=dropout,
                name="forward_representation_dropout")
        backward_representation = tf.nn.dropout(
                backward_representation,
                keep_prob=dropout,
                name="backward_representation_dropout")
            
        return forward_representation, backward_representation
            
    fw_output, bw_output = create_bi_directional_encoder(rnn_cell)
    return fw_output, bw_output

def create_tied_encoder(embedding_chars1, embedding_chars2, sequence_length1, sequence_length2, 
                dropout_keep_prob, hidden_units, is_train, rnn_cell, input_scope):
    with tf.variable_scope(input_scope+"_tied_encoder") as scope:
        anchor_encoder_fw, anchor_encoder_bw = BiRNN(embedding_chars1, dropout_keep_prob, input_scope+"_tied_encoder", 
                                      sequence_length1, hidden_units, is_train, rnn_cell)
        scope.reuse_variables()
        check_encoder_fw, check_encoder_bw = BiRNN(embedding_chars2, dropout_keep_prob, input_scope+"_tied_encoder", 
                                      sequence_length2, hidden_units, is_train, rnn_cell)

    return anchor_encoder_fw, anchor_encoder_bw, check_encoder_fw, check_encoder_bw
