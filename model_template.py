...
class ModelTemplate(object):
    ...
    def build_placeholder(self, config):
        ...
        # ---- place holder -----
        self.sent1_token = tf.placeholder(tf.int32, [None, self.max_length], name='sent1_token')
        self.sent2_token = tf.placeholder(tf.int32, [None, self.max_length], name='sent2_token')
        ...
        self.sent2_len = tf.placeholder(tf.int32, [None], name='sent2_token_length')
    ...
    def build_op(self):
        ...
        elif self.config["optimizer"].lower() == "amsgrad":
            self.opt = AMSGrad(self.learning_rate)
    ...
    def step(self, sess, batch_samples, dropout_keep_prob):
        ...
        [loss, train_op, global_step, 
            accuracy, preds] = sess.run([self.loss, self.train_op, 
                                          self.global_step, 
                                          self.accuracy, 
                                          self.pred_probs], 
                                          feed_dict=feed_dict)
