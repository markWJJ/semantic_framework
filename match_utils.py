...
def cal_max_question_representation(question_representation, atten_scores):
	atten_positions = tf.cast(tf.argmax(atten_scores, axis=2), tf.int32)  # [batch_size, passage_len]
	...
	with tf.variable_scope(scope or "match_passage_with_question"):
		...
		if with_full_match:
			...
			(attentive_rep, match_dim) = multi_perspective_match(context_lstm_dim,
								passage_reps, question_full_rep, is_training=is_training, dropout_rate=dropout_rate,
		...
		if with_attentive_match:
			...
			(attentive_rep, match_dim) = multi_perspective_match(context_lstm_dim,
					passage_reps, att_question_contexts, is_training=is_training, dropout_rate=dropout_rate,
		...
		if with_max_attentive_match:
			...
			(max_attentive_rep, match_dim) = multi_perspective_match(context_lstm_dim,
					passage_reps, max_att, is_training=is_training, dropout_rate=dropout_rate,
...
def bilateral_match_func(in_question_repres, in_passage_repres,
						question_lengths, passage_lengths, question_mask, passage_mask, input_dim, is_training, options=None, dropout_rate=0):
	...
	(match_reps, match_dim) = match_passage_with_question(in_passage_repres, in_question_repres, passage_mask, question_mask, passage_lengths,
								...
								is_training=is_training, options=options, dropout_rate=dropout_rate, forward=True)
	...
	(match_reps, match_dim) = match_passage_with_question(in_question_repres, in_passage_repres, question_mask, passage_mask, question_lengths,
								...
								with_attentive_match=options["with_attentive_match"],
								with_max_attentive_match=options["with_max_attentive_match"],
								is_training=is_training, options=options, dropout_rate=dropout_rate, forward=False)
	...
	question_aware_representatins = tf.nn.dropout(question_aware_representatins, dropout_rate)
	passage_aware_representatins = tf.nn.dropout(passage_aware_representatins, dropout_rate)
